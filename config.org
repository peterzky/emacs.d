...
(define-key evil-emacs-state-map (kbd "s-SPC") peter/leader-map)
...
(setq org-columns-default-format
      "%6CATEGORY %50ITEM %6CLOCKSUM %10TODO")
...
(defun peter/org-narrow-subtree ()
  "narrow to subtree if current entry has NEXT keyword"
  (if (string= (org-entry-get nil "TODO") "NEXT")
...
(add-hook 'org-clock-goto-hook 'peter/org-narrow-subtree)
(advice-add 'org-agenda-switch-to :after #'peter/org-narrow-subtree)
...
(setq org-todo-keywords
      '((sequence "NEXT(n)" "WAIT(w@/!)" "|" "DONE(d)" "CANCELED(c@)")
        (sequence "TODO(t)" "PROJ(p)" "MAYBE(m)" "|" "FINISH(f)")
        (sequence "REPEAT(r)" "BUG(b)" "|" "REF(R)" "FIXED(x)")))
...
(defun peter/run-link-open (cmd)
  (call-process-shell-command (format "%s &" cmd) nil 0))

(org-link-set-parameters "run" :follow 'peter/run-link-open)
...
(defun peter/org-agenda-quit-delete-frame (orig-fun &rest args)
  (if (equal "org-agenda" (frame-parameter nil 'name))
      (delete-frame)
    (call-interactively orig-fun)))

(advice-add 'org-agenda-quit :around #'peter/org-agenda-quit-delete-frame)
