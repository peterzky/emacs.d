...
(defvar my:theme 'doom-tomorrow-night)
...
(use-package popwin
  ...
  (push '(racer-help-mode :noselect nil) popwin:special-display-config)
  (push '(helpful-mode :noselect nil) popwin:special-display-config)
...
(add-to-list 'evil-motion-state-modes 'racer-help-mode)
...
(defun peter/go-install-or-run ()
  (interactive)
  (when (bound-and-true-p go-playground-mode) (go-playground-exec))
  (if (string= (buffer-substring-no-properties 1 13) "package main")
      (go-run)
    (compile "go install")))

(defun peter/go-install ()
  (interactive)
  (compile "go install"))
...
(use-package company-nix-options
  :ensure t)
#+END_SRC
** Rust
